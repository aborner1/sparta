...
double CollideVSS::rotrel_serial(int isp, double Ec)
{
  ...
  double rotphi = params[isp][isp].rotc1 / 
	  (1.0+params[isp][isp].rotc2/sqrt(Tr) + params[isp][isp].rotc3/Tr);
  return rotphi;
}
...
double CollideVSS::vibrel_serial(int isp, double Ec)
{
  ...
  double vibphi = 4.0 * MY_PIS * pow(diam,2.0)*pow(tref,omega)
                  * pow(Tr,-omega-0.5)*101325.0*exp(params[isp][isp].vibc1
                  * (pow(Tr,-1.0/3.0)-params[isp][isp].vibc2)-18.42)/sqrt(species[isp].mass*update->boltz);
  return vibphi;
}
...
{
  Particle::Species *species = particle->species;
  double rotphi = params[isp][isp].rotc1 / (1.0 + tgamma(species[isp].rotdof
                       + 2.5-params[isp][isp].omega)/tgamma(species[isp].rotdof+1.5-params[isp][isp].omega)
                       * params[isp][isp].rotc2*sqrt(update->boltz/Ec) + tgamma(species[isp].rotdof+2.5-params[isp][isp].omega)
                       /tgamma(species[isp].rotdof+2.0-params[isp][isp].omega)*params[isp][isp].rotc3*update->boltz/Ec);
  return rotphi;
}
...
double CollideVSS::vibrel_prohibdouble(int isp, double Ec, double doftot)
{
  ...
  double Zmw = 4.0*MY_PIS * pow(params[isp][isp].diam,2.0)*pow(params[isp][isp].tref,params[isp][isp].omega)
                  * pow(Tr,-params[isp][isp].omega-0.5)*101325.0*exp(params[isp][isp].vibc1
                  * (pow(Tr,-1.0/3.0)-params[isp][isp].vibc2)-18.42)/sqrt(species[isp].mass*update->boltz);
...
void CollideVSS::read_param_file(char *fname)
{
  ...
  if (relaxflag == VARIABLE) {
    if (relaxtypeflag == PROHIBDOUBLE) REQWORDS = 10;
    REQWORDS = 9;
  }
  ...
  while (fgets(line,MAXLINE,fp)) {
    ...
    if (jsp < 0 ) {
      ...
      if (relaxflag == VARIABLE) {
        ...
        if (relaxtypeflag == PROHIBDOUBLE) params[isp][isp].park = atof(words[9]);
      }
    }else {
      ...
      if (relaxflag == VARIABLE) {
        ...
        if (relaxtypeflag == PROHIBDOUBLE) params[isp][jsp].park = params[jsp][isp].park= atof(words[9]);
      }
    }
  }
